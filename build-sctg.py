...
def sctgdesk_customization(args):
    ...
    if not args.no_customization:
        ...
        replace_in_file('flutter/web/v1/js/src/connection.ts', 'const PORT = 21116', 'const PORT = 21118')
        replace_in_file('flutter/web/v1/js/src/connection.ts', 'uri = tmp[0] + ":" + (port + (isRelay ? roffset || 3 : 2));','uri = tmp[0] + ":" + (PORT + (isRelay ? 3 : 2));')
        replace_in_file('flutter/web/v1/js/src/connection.ts', 'ws://', 'wss://')
        ...
        replace_in_file('flutter/lib/desktop/pages/desktop_home_page.dart', "contains('rustdesk')", f"contains('{APP_NAME}')".lower())
